...
const MergedPlanes = forwardRef<THREE.Mesh, { material: THREE.ShaderMaterial; count: number; width: number; height: number }>(
  ({ material, width, count, height }, ref) => {
    ...
    const geometry = useMemo(() => {
      ...
      const totalW = count * width;

      for (let i = 0; i < count; i++) {
        const x = -totalW / 2 + i * width;
...
const Beams = ({
  ...
  const actualBaseColor = baseColor || (background !== 'transparent' ? background : '#000000');
  ...
  const beamMaterial = useMemo(() =>
    extendMaterial(THREE.MeshStandardMaterial, {
      ...
      material: { fog: true },
      uniforms: {
        diffuse: new THREE.Color(actualBaseColor),
        time: { value: 0 },
        roughness: 0.3,
        metalness: 0.3,
        uSpeed: { value: speed },
    ...
    }), [speed, noiseIntensity, scale, actualBaseColor]);

  return (
    <div className="absolute inset-0 w-full h-full overflow-hidden">
      <Canvas dpr={[1, 2]} style={{ background }}>
        <PerspectiveCamera makeDefault position={[0, 0, 15]} fov={45} />
        <ambientLight intensity={0.5} />
        <directionalLight color={lightColor} intensity={2} position={[0, 5, 10]} />
